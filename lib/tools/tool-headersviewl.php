<?php
 /** Модуль управления инструментом `получение заголовка сайта`
 * @author [Eugene]
 * @copyright 2011
 ...
 */
 ...
 class w_toolitem_headersviewl extends w_tools_def_mass_ajax {
  const W_COUNT_OF_URL_ANALISYS = 10;
  const W_SLEEP_INTERVAL = 0.5;  
  const W_PAYTRANSACTIONNUMBER = 5; /* не изменять */
  protected 
   $result;
  ...
  function _DoActionThisTool() {
   ...
   $http->connect_follow_redirect = $this->CheckPostValue('followredirect');  
   //результат запроса
   $this->result = array(
   ...
   if ($this->result['result']) {
	...
	$this->result['head']  = $http->res_header_source;	
   }   
   $this->result['error']     = $http->res_error;
   $this->result['httpcode']  = $http->res_http_code;
   //source
   $this->PrintDefaultSourceDataInfo('tpl_headersviewl_t_r.tpl', 'tpl_headersviewl_t_r_add_row.tpl', true); 	
   exit;	
  }//_DoActionThisTool  
