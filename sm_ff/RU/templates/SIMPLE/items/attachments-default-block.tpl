...
*}
 ...
 <div style="margin-top: 25px; border: 1px solid #B1B1B1; padding: 3px; padding-bottom: 6px;">
  ...
  {foreach from=$attfileslist2 key=nm item=val name=val}
   ...
   <a title="Перейти на страницу скачивания файла" href="{$smarty.const.W_SITEPATH}download/{$filesid}/{$objectid}/{$val2.iditem}">{$val2.fname}</a><label style="margin-left: 3px; font-size: 95%">({$CONTROL_OBJ->GetStrSizeFromBytes($val2.fsize)}{if $val2.shcountw}, загрузок: {$val2.dwcount}{/if})</label>
   </label>
   {if $val2.filetip}
