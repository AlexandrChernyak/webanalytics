   ...
   {if !$PANEL_CONTROL->line_obj}?{else}
	...
	{elseif $PANEL_CONTROL->line_obj->GetCurrentValue($val.sid, $val.data, 'value') !== false}
	  ...
	  {elseif $val.data.type == '3'}
	   ...
	   <a title="История изменения показателей" href="javascript:" onclick="ProcessViewHistoryElement('{$PANEL_CONTROL->GetResult('urllineinfo.iditem')}')"><img alt="h" src="{$smarty.const.W_SITEPATH}css/panel/img/history.png" width="16" height="16" align='absmiddle'></a>
	   
	   <span style="margin-left: 3px"><a title="Выполнить анализ сайта" target="_blank" href="{$smarty.const.W_SITEPATH}tools/analysis/{$PANEL_CONTROL->GetResult('urllineinfo.urlid')}"><img alt="a" src="{$smarty.const.W_SITEPATH}css/panel/img/info.png" width="16" height="16" align='absmiddle'></a></span>
