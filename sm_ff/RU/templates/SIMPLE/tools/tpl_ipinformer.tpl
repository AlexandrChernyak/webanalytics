...
<div style="margin-top: 5px">
 ...
 Данный инструмент предоставляет Вам возможность создать информер Вашего IP адреса для Вашего сайта, или подписи на форуме (блоге).
 {/if} 
 ...
 <form method="post" name="tollform" id="toolform"{if $smarty.post.doactiontool == 'do'} onsubmit="return GetInformCodeCheck(this)"{/if}>
  ...
  <div class="typelabel" style="margin-top: 13px; font-size: 16px"> 
   Ваш IP: <b style="border-bottom: 1px dashed #0000FF">{$tool_object->GetResult('ip')}</b>
  </div> 
...
<a href="http://{$smarty.const.W_HOSTMYSITE}" target="_blank" title="Посетить {$smarty.const.W_HOSTMYSITE}"><img border="0" src="http://{$smarty.const.W_HOSTMYSITE}/informer-images/2/image-{$tool_object->GetResult('newinf.iditem')}.tif" alt="IP informer"></a>
