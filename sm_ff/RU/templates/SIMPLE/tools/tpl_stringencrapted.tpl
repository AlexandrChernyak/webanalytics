...
<div style="margin-top: 5px">
 ...
 Данный инструмент поможет Вам получить хэш строки различных типов и размеров.
 {/if}
 ...
 {if $smarty.post.doactiontool == 'do' && isset($tool_object)}
  ...
  <div style="margin-top: 25px; padding: 2px 0 10px 0; ">
   ...
   {if !$tool_object->GetResult()}
    <div style="margin-left: 8px; color: #FF0000">Нет данных!</div>
   {else}
    {foreach from=$tool_object->GetResult() item=val name=val}
     <div style="margin-top: 18px; overflow: auto; height: auto; width: 550px">
	  <div>Тип хэша: "{$val.name}", Размер: "{$val.size} Byte"</div>
