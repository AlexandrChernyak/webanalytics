...
<div>
 <a href="{$smarty.const.W_SITEPATH}account/ref-banner/"{if !$smarty.get.flash} style="color: #000000"{/if}>Обычные баннеры</a><label style="margin: 0 5px 0 5px">|</label><a href="{$smarty.const.W_SITEPATH}account/ref-banner/&flash=1"{if $smarty.get.flash} style="color: #000000"{/if}>Flash баннеры</a>
</div>
<div style="margin-top: 20px">
 ...
 {/literal}
 {if !$blist}<div style="margin-left: 4px">Нет активных баннеров!</div>{else}
  {foreach from=$blist key=rname item=val name=val}
 
   <div class="analisislabelid" style="margin-top: 15px; background: #F4F4F4"><strong>{$rname}</strong><label style="color: #000000; margin-left: 6px">[ <a style="font-size: 95%" href="javascript:" onclick="ShHdBlElementA(this, 'block_{$ref.iditem}')">Скрыть</a> ]</label></div>
   <div style="margin-top: 12px; width: 95%; padding-left: 6px" id="block_{$ref.iditem}">    
    ...
    <div style="padding-left: 2px">
     ...
     <div style="margin-top: 6px">
      <input type="checkbox" style="cursor: pointer" name="ch{$ref.iditem}" id="ch{$ref.iditem}" onclick="ShHideElementFlash(this, '{$ref.iditem}')" /><label for="ch{$ref.iditem}" style="cursor: pointer"> Показать код баннера</label>
     </div>
     ...
     <div id="rp{$ref.iditem}" style="visibility: hidden; display: none">
      <div style="margin-top: 15px; padding-left: 4px">
       ...
       <div>
        ...
        {if $ref.isflash}
        <div>
        <input type="radio" style="cursor: pointer" name="sel{$ref.iditem}" id="sel1{$ref.iditem}" onclick="GetCodeElement('{$ref.iditem}', 'lp1', '{$ref.isflash}')" /><label for="sel1{$ref.iditem}" id="lp1{$ref.iditem}" style="cursor: pointer; padding-left: 4px">Как указано в ролике</label>
        </div>
        {else}      
