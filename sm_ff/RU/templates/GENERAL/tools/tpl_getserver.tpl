...
<div style="margin-top: 5px">
 ...
 Данный инструмент поможет Вам определить сервер, на котором находится указанный сайт.
 {/if}
 ...
 <form method="post" name="tollform" id="toolform" onsubmit="return PrepereToSend(this)">
  <div class="typelabel"><label id="red">*</label> Список сайтов или IP <label class="prep_label_analisys">(пример: <a href="javascript:" onclick="DoSetDefUrl('urls')">{$smarty.const.W_HOSTMYSITE}</a>, или <a href="javascript:" onclick="ClearVal('urls')">очистить</a> список)</label>{if isset($tool_object)}, &nbsp;Не более: {$tool_object->GetLimitCount()}{/if}</div>
  <div class="typelabel">  
 ...
 {if $smarty.post.doactiontool == 'do' && isset($tool_object)}
  ...
  {else}
   ...
   <tr>
	...
	<td class="h_td" valign="center" align="right" width="120px">IP сайта</td>		
	<td class="h_td2" valign="center" align="center"><label style="margin-right: 4px">Сервер сайта</label></td>
   </tr>   	
   {if $tool_object->GetResult()}
	{foreach from=$tool_object->GetResult() item=val name=val}	 
	 ...
	 <td class="sth1" valign="center" align="right" width="120px">
	 ...
	 <td class="sth1" valign="center" align="center" style="border-right: 1px solid #E4D9CB;">
	  <label style="margin-right: 4px">{if !$val.server}?{else}{$val.server}{/if}</label>
	 </td>
    </tr>	 
	{/foreach}
   {else}
