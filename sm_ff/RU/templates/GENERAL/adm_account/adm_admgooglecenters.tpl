...
<div>
<a href="{$smarty.const.W_SITEPATH}account/admgooglecenters&new=1"{if $smarty.get.new} style="color: #000000"{/if}>Добавить датацентр(ы)</a> | <a{if !$smarty.get.new} style="color: #000000"{/if} href="{$smarty.const.W_SITEPATH}account/admgooglecenters/">Все датацентры (<label style="color: #000000">{$global_googlecenters_count_info.all}</label>)</a>
</div>
...
{else}
 ...
 <form method="post" name="googledcformadd" id="googledcformadd" onsubmit="return PrepereSend(this)">
  ...
  <div class="typelabel">
   <input type="checkbox" {if $CONTROL_OBJ->CheckPostValue('enabled') || $smarty.post.googlecentersnew != 'do'} checked="checked" {/if}style="cursor: pointer" name="enabled" id="enabled"><label for="enabled" style="cursor: pointer">&nbsp;Добавить активными</label>  
  </div>
