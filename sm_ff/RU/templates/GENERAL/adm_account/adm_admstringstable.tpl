...
<div>
<a href="{$smarty.const.W_SITEPATH}account/admstringstable&new=1{if $smarty.get.page}&oldpage={$smarty.get.page}{/if}"{if $smarty.get.new} style="color: #000000"{/if}>Добавить строку</a> | <a{if !$smarty.get.new} style="color: #000000"{/if} href="{$smarty.const.W_SITEPATH}account/admstringstable/{if $smarty.get.oldpage}&page={$smarty.get.oldpage}{/if}">Все строки (<label style="color: #000000">{$adm_object->GetResult('count')}</label>)</a>
</div>
   ...
   {if $adm_object->GetResult('data.source')}
	{foreach from=$adm_object->GetResult('data.source') item=val name=val}
	 ...
	 <td class="sth1" valign="center" align="left" width="200px" onclick="$('#chid{$smarty.foreach.val.index}').click()">
	  ...
	  <a href="{$smarty.const.W_SITEPATH}account/admstringstable&new=1&modify={$val.strident}{if $smarty.get.page}&oldpage={$smarty.get.page}{/if}" title="Изменить текст константы">{$val.strident}</a>	  
	  </span>	 
	 </td> 
	 ...
	 <td class="sth1" valign="center" align="center" style="border-right: 1px solid #E4D9CB;" onclick="$('#chid{$smarty.foreach.val.index}').click()">
	 <span style="margin-left: 3px">	  
	  {if $val.labelid}{$val.labelid}{else}<em>(нет метки)</em>{/if}	  
	  </span>
	 </td>
...
{else}
 ...
 <form method="post" name="addnewstring" id="addnewstring" onsubmit="return PrepereSent(this)">
  ...
  {/if}
  <div class="typelabel"><label id="red">*</label> Идентификатор строки{if !$adm_object->GetResult('modifyinfo')} (до 120 символов){/if}</div>
  <div class="typelabel">
   {if !$adm_object->GetResult('modifyinfo')}
 ...
 <div class="typelabel" style="margin-top: 15px">
  <input type="submit" value="&nbsp;{if !$adm_object->GetResult('modifyinfo')}Добавить строку{else}Сохранить изменения{/if}&nbsp;" class="button" name="rb" id="rb">
 </div>
 ...
 <div style="margin-top: 10px">
  ...
  {else}
   <label style="color: #008000">Строка успешно {if !$adm_object->GetResult('modifyinfo')}добавлена{else}изменена{/if}!</label>
  {/if}
 </div>
 {/if}
{/if}
