...
<div style="margin-top: 4px">
 ...
 {foreach from=$listtoolsX item=val name=val} 
  <div style="margin-top: 18px; border-bottom: 1px solid #969696; padding-bottom: 5px;"><b>{$val.group.name}</b><label style="color: #000000; margin-left: 6px">[ <a style="font-size: 95%" href="javascript:" onclick="ShHdBlElementA(this, 'block_{$val.group.iditem}')">Hide</a> ]</label>
  </div>
 ...
 Tool: <a href="{$smarty.const.W_SITEPATH}tools/{$smarty.get.toolid}/" target="_blank">{$CONTROL_OBJ->GetText($adm_object->GetToolParam($smarty.get.toolid, 'descr'))}</a>
 </div>   
  
 <div> 
  <div class="typelabel" style="background: #F3F5F5; padding: 3px"><strong>Icon up to 16x16</strong></div>
  ...
  <form method="post" name="icon16" id="icon16" enctype="multipart/form-data" onsubmit="return DoSendForm(this)">
   ...
   <div class="typelabel"> Tool Icon (formats: {$adm_object->GetListTypes()}{if $adm_object->GetResult('maxsize16')}, max size: <b>{$adm_object->GetResult('maxsize16')}</b>){/if}</div>
   ...
   <div class="typelabel">
    <input type="checkbox" style="cursor: pointer" name="resizeimage16" id="resizeimage16"{if $adm_object->CheckPostValue('resizeimage16')} checked="checked"{/if} /><label for="resizeimage16" style="cursor: pointer">&nbsp;Reduction of up to 16x16 if height or width greater than 16px</label>
   </div>
  ...
  {if $smarty.post.doloadimage16 == 'do'}
   <div style="margin-top: 6px">
    ...
    {else}
      <div style="color: #008000">Icon has been successfully installed!</div>
    {/if}
   </div>
  {/if}
 </div> 
 ...
 <div style="margin-top: 25px"> 
  <div class="typelabel" style="background: #F3F5F5; padding: 3px"><strong>Icon up to 64x64</strong></div>
  <div style="margin: 6px 0 6px 0">
  ...
  <form method="post" name="icon64" id="icon64" enctype="multipart/form-data" onsubmit="return DoSendForm(this)">
   ...
   <div class="typelabel"> Tool Icon (formats: {$adm_object->GetListTypes()}{if $adm_object->GetResult('maxsize64')}, max size: <b>{$adm_object->GetResult('maxsize64')}</b>){/if}</div>
   <div style="font-size: 95%">To delete an image - leave blank.</div>
   <div class="typelabel">
   ...
   <div class="typelabel">
    <input type="checkbox" style="cursor: pointer" name="resizeimage64" id="resizeimage64"{if $adm_object->CheckPostValue('resizeimage64')} checked="checked"{/if} /><label for="resizeimage64" style="cursor: pointer">&nbsp;Reduction of up to 64x64 if height or width greater than 64px</label>
   </div>
