...
<div>
<a href="{$smarty.const.W_SITEPATH}account/admfontssection&new=1"{if $smarty.get.new} style="color: #000000"{/if}>Add Font</a> | <a href="{$smarty.const.W_SITEPATH}account/admfontssection/"{if !$smarty.get.new} style="color: #000000"{/if}>All fonts (<label style="color: #000000">{$adm_object->GetResult('count')}</label>)</a>
</div>
...
<script type="text/javascript">
 ...
 function PrepereSend(th) {
  ...
  if (count <= 0 && th.actionlistmakes.value != 'all' && th.actionlistmakes.value != 'dall') { 
   alert('Highlight at least one font file!'); return false; 
  }
  th.actioncountmess.value = count;
  if (th.actionlistmakes.value == 'delete') {
   if (!confirm('Do you really want to delete ['+count+'] files?')) { return false; }
  } else 
  if (th.actionlistmakes.value == 'enabled') {
   if (!confirm('Do you really want to activate ['+count+'] fonts?')) { return false; }
  } else
  if (th.actionlistmakes.value == 'disabled') {
   if (!confirm('Do you really want to deactivate ['+count+'] fonts?')) { return false; }
  } else
   if (th.actionlistmakes.value == 'dall') {
    if (!allsaveenabled) { alert('No data for delete!'); return false; }	
    if (!confirm('Do you really want to delete all fonts?')) { return false; }	
...
<form method="post" name="fontsform" id="fontsform" onsubmit="return PrepereSend(this)">
 ...
 <table width="100%" cellpadding="0" cellspacing="0" border="0">
   <tr>
	...
	<td class="h_td" valign="center" align="left"><span style="margin-left: 3px">Font</span></td>	
   ...
   {if $adm_object->GetResult('data.source')}
	{foreach from=$adm_object->GetResult('data.source') item=val name=val}
	 ...
	 <td class="sth1" valign="center" align="left" onclick="$('#chid{$smarty.foreach.val.index}').click()">
	  ...
	  <span style="position: relative; top: -2px"><a href="{$smarty.const.W_SITEPATH}account/admfontssection&downloadfile={$val.iditem}" title="Download file">{$val.fontname}</a></span>
	  </span>	 
	 </td> 
	 ...
	 <td class="sth1" valign="center" align="center" onclick="$('#chid{$smarty.foreach.val.index}').click()" width="80px">
	  {if !$val.fontuse}<i>(no)</i>{else}<i style="color: #333399">(active)</i>{/if}	  	 
	 </td>
...
{else}
 ...
 <form method="post" name="ffadd" id="ffadd" enctype="multipart/form-data" onsubmit="return PrepereToSend(this)"> 
  ...
  <div class="typelabel"><label id="red">*</label> Font file (formats: {$adm_object->GetListTypes()}{if $adm_object->GetResult('maxsize')}, max. size: <b>{$adm_object->GetResult('maxsize')}</b>){/if}</div>  
  <div class="typelabel"> 
 ...
 <div class="typelabel"><input type="submit" value="&nbsp;Add font file&nbsp;" class="button" name="rb" id="rb"></div>
 </form>
 ...
 <div style="margin-top: 10px">
  ...
  {else}
   <label style="color: #008000">File successfully added!</label>
  {/if}
 </div>
 {/if}
{/if}
