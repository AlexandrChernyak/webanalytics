...
<tr onmouseover="DoHigl(this, 1)" onmouseout="DoHigl(this, 0)">
 <td class="sth1" valign="center" align="left" style="border-left: 1px solid #E4D9CB{if $tool_object->GetCurrentIndex() > 1}; border-top: 1px solid #C9D5FC{/if}">
  ...
  <div>Redirecting to <a href="{$tool_object->GetResultValue('redil')}" target="_blank">{$tool_object->CorrectURLLink($tool_object->GetResultValue('redil'), 50)}</a></div>
  <div style="margin-top: 3px">
   <div>
   Total redirects: <b>{$tool_object->GetRedirectCount()}</b>, <label class="prep_label_analisys"><a id="shblockid" style="cursor: pointer" onclick="ShowHideBlockItem('shblockid_data{$tool_object->GetCurrentIndex()}', this)">Show List</a></label>
   </div>
   <div id="shblockid_data{$tool_object->GetCurrentIndex()}" style="display: none; visibility: hidden; margin-top: 4px">
    <div style="margin-top: 4px; border-top: 1px dashed #C0C0C0; padding-top: 4px">
	 {foreach from=$tool_object->redirectlist item=val name=val}
	  ...
	  <b>№ {$tool_object->GetIndexNumber($smarty.foreach.val.index)}</b>, Response code: <b>{$val.code}</b>
	  </div>
	  <div>
	  Go to: <a href="{$val.link}" target="_blank">{$tool_object->CorrectURLLink($val.link, 50)}</a>
	  </div>
	  <div style="margin-top: 10px"><b>Response header:</b></div>
