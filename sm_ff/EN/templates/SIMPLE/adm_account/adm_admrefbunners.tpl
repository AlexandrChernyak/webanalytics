...
<div>
<a href="{$smarty.const.W_SITEPATH}account/admrefbunners&new=1{if $smarty.get.page}&oldpage={$smarty.get.page}{/if}"{if $smarty.get.new} style="color: #000000"{/if}>Add Banner</a> | <a{if !$smarty.get.new} style="color: #000000"{/if} href="{$smarty.const.W_SITEPATH}account/admrefbunners/{if $smarty.get.oldpage}&page={$smarty.get.oldpage}{/if}">All Banners (<label style="color: #000000">{$adm_object->GetResult('count')}</label>)</a>
</div>
...
<script type="text/javascript">
 ...
 function PrepereSend(th) {
  ...
  if (th.actionlistmakes.value == 'dall') {
   ...
   if (!confirm('Are you sure you want to delete all banners?')) { return false; }	
...
<form method="post" name="vlinksform" id="vlinksform" onsubmit="return PrepereSend(this)">
 ...
 <span style="margin-left: 8px">
  <input type="submit" value="&nbsp;Delete all banners&nbsp;" class="deletemessbut" name="adid" id="adid" onclick="SetActionP('dall')" style="//width: 160px;">
 </span>
 ...
 <table width="100%" cellpadding="0" cellspacing="0" border="0">
   <tr>
	...
	<td class="h_td2" valign="center" align="left"><span style="margin-left: 3px">Banner</span></td>
   </tr>	
   {literal}
   <style type="text/css">
   ...
   <script type="text/javascript">
    ...
    function ModifyElementItem(pID, pName) {
     if (inprocess) {
      return alert('Wait until...');  
     }   
     var res = prompt("Enter new value!", $('#'+pName+pID).text());
     if (res == null) { return ; }
     if (!res) { res = 0; } else {
      if (!IisInteger(res, true)) { return alert('The value must be a number!'); }  
     }
     $('#change'+pName+pID).html(
     '<label style="font-size: 95%; color: #0000FF">Applying, please wait..</label>'
     );
     inprocess = true;
     gID   = pID;
     gName = pName;
    ...
    function ContinueItem(data) {
     ...
     if (gID && gName) {        
      $('#change'+gName+gID).html(    
       '<a href="javascript:" onclick="ModifyElementItem(\''+gID+'\', \''+gName+'\')" style="font-size: 95%">Modify</a>'
      );
      if (data || data == '0') {         
       $('#'+gName+gID).html(data);       
   ...
   {if $adm_object->GetResult('data.source')}
	{foreach from=$adm_object->GetResult('data.source') item=val name=val}
	 ...
	 <td class="sth1" valign="top" align="left" style="padding: 3px; border-right: 1px solid #E3E4E8;" onclick="$('#chid{$smarty.foreach.val.index}').click()">
      ...
      <div style="margin-top: 4px">
       <div>FileName: <em style="color: #797979">{$val.bofilename}</em></div>
       <div>File Size: <em style="color: #797979">{$adm_object->GetImageFileSizeEX($val.bsize)}</em></div>
       <div>
        Height: <em style="color: #797979"><label id="h{$val.iditem}">{$val.bheight}</label>px</em>
        {if $val.isflash}
        <label id="changeh{$val.iditem}" style="margin-left: 6px">
        <a href="javascript:" onclick="ModifyElementItem('{$val.iditem}', 'h')" style="font-size: 95%">Modify</a>
        </label>        
        {/if}        
       </div>
       <div>
        Width: <em style="color: #797979"><label id="w{$val.iditem}">{$val.bwidth}</label>px</em>
        {if $val.isflash}
        <label id="changew{$val.iditem}" style="margin-left: 6px">
        <a href="javascript:" onclick="ModifyElementItem('{$val.iditem}', 'w')" style="font-size: 95%">Modify</a>
        </label>        
        {/if}        
       </div>
      </div>
...
{else}
 ...
 <form method="post" name="addnewlink" id="addnewlink" enctype="multipart/form-data" onsubmit="return PrepereSent(this)">
  ...
  <div class="typelabel"> Banner image (formats: {$adm_object->GetListTypes()}{if $adm_object->GetResult('maxsize')}, max size: <b>{$adm_object->GetResult('maxsize')}</b>){/if}</div>
  <div class="typelabel">
   <input type="file" size="20" style="width: 95%; height: 22px" class="inpt" name="image" id="image">
  </div> 
  
  <div class="typelabel">
   <input type="checkbox" style="cursor: pointer" name="isflashobject" id="isflashobject" onclick="ShHideElementFlash(this)" /><label for="isflashobject" style="cursor: pointer"> This place is a flash movie, set the width and height</label>
  </div>
  
  <div id="flashparams" style="visibility: hidden; display: none">
  <div class="typelabel">The width and height of the banner. If the value is `0` - the value will not be used, respectively movie will be extended automatically. Specifies only the number (without the px)</div>
  <div class="typelabel">
   <label>Height: </label><input type="text" class="inpt" style="width: 50px" name="hflash" id="hflash" value="90" maxlength="5"> <label style="margin-left: 6px">Width: </label><input type="text" class="inpt" style="width: 50px" name="wflash" id="wflash" value="400" maxlength="5">
  </div>
  </div>
 ...
 <div style="margin-top: 10px">
  ...
  {else}
   <label style="color: #008000">Banner successfully added!</label>
  {/if}
 </div>
 {/if}
{/if}
