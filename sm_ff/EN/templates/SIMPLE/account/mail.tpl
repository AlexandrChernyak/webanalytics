{literal}
 <script type="text/javascript">
  ...
  function PrepereSend(th) {
   ...
   if (count <= 0) { alert('Mark at least one message!'); return false; }
   th.actioncountmess.value = count;
   if (th.actionmailslist.value == 'delete') {
	if (!confirm('Do you really want to delete ['+count+'] messages?')) { return false; }
   } else if (th.actionmailslist.value == 'read') {
	if (!confirm('Do you really want to mark read ['+count+'] messages?')) { return false; }
   } else { alert('The operation is not defined! Perhaps disabled in your browser JavaScript!'); return false; }
   return true;   	
  }//PrepereSend
  function SetEnabled() {
	var count = GetSelCount();
...
{/if}
<div>
<a href="{$smarty.const.W_SITEPATH}account/mail/new/">Create a new message</a> | 
<a {if !$smarty.get.send}style="color: #000000"{/if}href="{$smarty.const.W_SITEPATH}account/mail/">Incoming threads({if $global_user_info.privatenew}<label id="red">{$global_user_info.privatenew}</label>/{/if}<label style="color: #000000">{$global_user_info.privateall}</label>)</a> | 
<a {if $smarty.get.send}style="color: #000000"{/if}href="{$smarty.const.W_SITEPATH}account/mail/&send=1">Outgoing threads (<label style="color: #000000">{$global_user_info.privatesend}</label>)</a> 
</div>
{if !$smarty.get.send}
<div style="margin-top: 8px">
<input type="submit" value="&nbsp;Delete&nbsp;" class="deletemessbut" name="did" id="did" onclick="SetActionP('delete')">
<span style="margin-left: 12px">
<input type="submit" value="&nbsp;Mark unread&nbsp;" class="readmessbut" name="rid" id="rid" onclick="SetActionP('read')">
</span>
</div>
{/if}
<div style="margin-top: 12px">
 ...
 <table width="100%" cellpadding="0" cellspacing="0" border="0">
   <tr>
	...
	{/if}
	{if !$smarty.get.send}
	<td class="h_td" valign="center" align="center" width="80px">From</td>
	{else}
	<td class="h_td" valign="center" align="center" width="80px">To</td>
	{/if}
	<td class="h_td" valign="center" align="left" style="padding-left: 4px">Subject</td>
	<td class="h_td" valign="center" align="center" width="80px">Messages</td>
   ...
   {if $global_user_mail_info && $global_user_mail_info.source}
	{foreach from=$global_user_mail_info.source item=val name=val}
	 ...
	 <td class="sth1" valign="center" align="center" width="130px" style="border-right: 1px solid #E3E4E8;" {if !$smarty.get.send}onclick="$('#chid{$smarty.foreach.val.index}').click()"{/if}>
	 {$val.dateadd}
	 </td>
    </tr>  
    {if !$smarty.get.send}
